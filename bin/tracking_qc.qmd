...
df_raw <- read.csv(params$trackmate_fn)
df_filtered <- read.csv(params$trackmate_filtered_fn)
```
...
n_duplicates <- duplicate_trackids |>
    nrow()
track_ids <- if (n_duplicates == 0) '' else sprintf(" with `TRACK_ID`s %s", paste(as.character(sort(unique(duplicate_trackids$TRACK_ID))), collapse=", "))
```
...
@fig-cell-positions shows the positions of the tracked and filtered cells over the duration of the timelapse.

```{r fig-cell-positions, fig.cap="Cell positions throughout the timelapse."}
df_filtered |>
    ...
    ungroup() |>
    arrange(FRAME) |>
    ggplot(aes(x=POSITION_X, y=POSITION_Y, group=TRACK_ID, colour=FRAME)) +
	geom_line() +
	scale_colour_viridis_c("Frame") +
